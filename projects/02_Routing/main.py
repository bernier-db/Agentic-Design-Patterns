...
class RoutingWorkflow:
    ...
    def define_branches(self):
        ...
        self.branches = {
            "weather": RunnablePassthrough.assign(output=lambda x: self.weather_handler(x["request"]["request"])),
            "news": RunnablePassthrough.assign(output=lambda x: self.news_handler(x["request"]["request"])),
            "stock_market": RunnablePassthrough.assign(output=lambda x: self.stock_market_handler(x["request"]["request"])),
            "unclear": RunnablePassthrough.assign(output=lambda x: self.unclear_handler(x["request"]["request"])),
        }
    ...
    def run_coordinator_agent(self, request: str):
        ...
        } | self.delegation_branch | (lambda x: x["output"])
    ...
    def create_delegation_branch(self) -> RunnableBranch:
        ...
        return RunnableBranch(
            ...
            (lambda x: x['decision'].strip() == "stock_market", self.branches["stock_market"]),
